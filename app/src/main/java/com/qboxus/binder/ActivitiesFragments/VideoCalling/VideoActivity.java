package com.qboxus.binder.ActivitiesFragments.VideoCalling;
...
import com.qboxus.binder.ActivitiesFragments.VideoCalling.util.CameraCapturerCompat;
...
public class VideoActivity extends AppCompatActivity {
    ...
    public void showCallingDialog() {
        ...
        if(callerImage !=null && !callerImage.equals("")) {
            Uri uri;
            if(callerImage.contains("http")) {
                uri = Uri.parse(callerImage);
            }else {
                uri = Uri.parse(Constants.BASE_URL + callerImage);
            }
    ...
    private void removeRemoteParticipant(RemoteParticipant remoteParticipant) {
        ...
        if(remoteParticipant.getIdentity().contains(callerId)){
            Log.d(Constants.tag, "remoteParticipant.getIdentity().contains(callerId)");
            Toast.makeText(this, callerName +" hang out the call", Toast.LENGTH_SHORT).show();
            finish();
        }
        ...
        return new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (room != null) {
                    ...
                    minutes = minutes + 1;
                    Log.d(Constants.tag, "Minutes: "+minutes);
                    Functions.getSharedPreference(VideoActivity.this).edit().putInt(Variables.audioCallingUsedTime, 0).commit();
                    Functions.getSharedPreference(VideoActivity.this).edit().putInt(Variables.videoCallingUsedTime, 0).commit();
                }
    ...
    private void retrieveAccessTokenfromServer() {
         ...
         StringRequest postRequest = new StringRequest(Request.Method.GET,
                Constants.TWILIO_ACCESS_TOKEN_SERVER +identity+"?roomname="+ roomName,
    ...
    public void Send_notification(String call_status, String message){
        ...
        ApiRequest.sendNotification(this, json, new Callback() {
            @Override
    ...
    public void Start_limit_Timer(){
        if(!Functions.getSharedPreference(VideoActivity.this).getBoolean(Variables.isProductPurchase, false)){
